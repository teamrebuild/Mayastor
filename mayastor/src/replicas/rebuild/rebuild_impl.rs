...
impl RebuildJob {
        ...
        while self.tasks.active > 0 {
            match self.await_one_task().await {
                Some(r) => match r.error {
                    None => {
                        if self.states.pending_not(RebuildState::Running) {
    ...
    fn reconcile_to_state(&mut self, state: RebuildState) -> bool {
        if self.states.pending_is(state) {
            self.reconcile();
            true
        } else {
            false
...
impl RebuildStates {
    ...
    fn pending_not(&self, state: RebuildState) -> bool {
        self.pending.is_some() && self.pending != Some(state)
    }

    fn pending_is(&self, state: RebuildState) -> bool {
        self.pending == Some(state)
    }
