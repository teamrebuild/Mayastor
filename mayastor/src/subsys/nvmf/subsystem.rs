...
use crate::{
    ...
    subsys::{
        nvmf::{transport::TransportID, Error, Target, NVMF_TGT},
        Config,
    },
};
...
impl NvmfSubsystem {
    ...
    pub fn stop_all(tgt: &Target) {
        extern "C" fn stop_next(
            ss: *mut spdk_nvmf_subsystem,
            _arg: *mut c_void,
            _status: i32,
        ) {
            unsafe {
                let ss = spdk_nvmf_subsystem_get_next(ss);
                if !ss.is_null() {
                    spdk_nvmf_subsystem_stop(
                        ss,
                        Some(stop_next),
                    ...
                    NVMF_TGT.with(|t| t.borrow_mut().next_state())
                }
            }
        }
        unsafe {
            let ss = spdk_nvmf_subsystem_get_first(tgt.tgt_as_ptr());
            if ss.is_null() {
